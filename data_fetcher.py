...
def get_match_page_urls(tournament_id: int) -> List[str]:
    ...
    for link in soup.find_all('a', href=True):
        href = link.get('href')
        if href and re.match(r'^/\d+/', href) and '/event/' not in href:
            matches.append(f"{VLR_BASE_URL}{href}")
...
def is_map_live(game_element) -> bool:
    ...
    return 'active' in game_element.get('class', [])
...
def parse_match_page(html: str, match_url: str) -> Tuple[List[Dict], str, Optional[str]]:
    ...
    match_id = None
    match_search = re.search(r'/(\d+)/', match_url)
    if match_search:
...
if __name__ == '__main__':
    import sys
    tid = int(sys.argv[1]) if len(sys.argv) > 1 else 1923
